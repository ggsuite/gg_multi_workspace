...
/// ticket (`path: ../<repo>` / `link:../<repo>`, or `link:./.gg/ts_links/…`
/// for a shim leading to the sibling's sources). When a repo leaves the
...
/// and so does the shim. A file that holds nothing but the removed entries
/// is deleted instead of left behind as an empty override.
///
...
List<Directory> removeDependencyOverrides({
  ...
  for (final repoDir in repoDirs) {
    ...
    // The shim a pnpm override linked the removed repo through (see
    // TsLinkShims) is garbage once the override is gone.
    for (final name in packageNames) {
      const TsLinkShims().remove(projectDir: repoDir, name: name);
    }
