...
**Unresolvable dependency**: the transitive scan turns every dependency of an ocean repo into a repository name (`@tssuite/base-dna` → `base-dna`), so a package that was renamed or is not published under its repository name ends up as a repo nobody owns. The report therefore names the dependency *as the manifest writes it*, the manifest path, and `--no-transitive` as the way to keep working while the manifest is fixed (`_missingDependencyHint`, handed to `addRepositoryHelper` as its `failureHint`).
