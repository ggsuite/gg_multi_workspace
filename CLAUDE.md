...
By default a ticket also receives the repos that lie *between* its repos in the dependency graph (`a → b → c`, adding `a` and `c` pulls `b` in), which is what makes the localized refs resolve. **`--no-transitive-repos`** copies only the requested repos: the between-nodes are left out, and with them the two steps that only serve them — cloning the missing transitive deps into the ocean and building the dependency graph — are skipped as well.
